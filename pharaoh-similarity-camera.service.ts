...
export class PharaohSimilarityCameraService {
  ...
  private faceDetectionScriptPromise: Promise<void> | null = null;
  ...
  private async initializeMediaPipe(): Promise<void> {
    if (!this.detector) {
      ...
      // Try SIMD first, fall back to non-SIMD if WASM aborts
      let detector: FaceDetectionType | null = null;
      let forceNonSimd = false;
      ...
      try {
        ...
        console.warn(
          'MediaPipe SIMD WASM failed, falling back to non-SIMD:',
          simdError
        );
        forceNonSimd = true;
        detector = null;
  ...
  private loadFaceDetectionScript(): Promise<void> {
    ...
    // Already available globally (e.g. script was included in index.html)
